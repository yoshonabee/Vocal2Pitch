    ...
    def _run_one_epoch(self, epoch, valid=False):
        ...
        with tqdm(enumerate(dataloader), total=math.ceil(len(dataloader.dataset) / dataloader.batch_size), unit="iter") as t:
            for i, (data) in t:
                ...
                loss, pred, y, onset_loss, pitch_loss = self._loss_fn(self.model, data)
                ...
                total_scores['onset_loss'] += onset_loss.item() * y.size(0)
                total_scores['pitch_loss'] += pitch_loss.item() * y.size(0)

                if not valid:
                    avg_onset_loss = total_scores['onset_loss'] / total_count
                    avg_pitch_loss = total_scores['pitch_loss'] / total_count
                    self.writer.add_scalar("avg_onset_loss", avg_onset_loss, self.steps)
                    self.writer.add_scalar("avg_pitch_loss", avg_pitch_loss, self.steps)
